  ...
  window.addEventListener("message", (event) => {
    ...
    switch (event.data.action) {
      ...
      case "snippetAPI.loadSnippet":
        ...
        //
        // `source.snippetUID` (below) is the source snippet's own id --
        // deliberately not reused as this snippet's own `id`, since a draft
        // id and a space snippet id are generated by the same scheme on the
        // website and aren't guaranteed disjoint. `id` here only ever needs
        // to be unique among *this extension's own* open snippets, which
        // Date.now() already is.
        //
        ...
        chrome.storage.sync.get({ snippets: [] }, (data) => {
          ...
          chrome.storage.sync.set({ snippets: updatedSnippets }, () => {
            ...
            chrome.storage.sync.set({
              ...
              // Resolve with the assigned id (rather than just `true`) so
              // the website has it immediately, without waiting on the
              // chrome.storage.onChanged -> snippetAPI.listener round trip.
              window.postMessage({ action: "snippetAPI.callback", id: event.data.id, result: { id: sdata.id } }, "*");
